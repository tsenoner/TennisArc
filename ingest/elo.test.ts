...
describe("parseEloTable", () => {
  ...
  it("parses overall + surface ELO, keyed by normalized name", () => {
...
function player(name: string): Player {
  return { id: name, name, country: "", seed: null, entry: null, ranking: null, sofaSlug: null, elo: null, birthdate: null };
}

describe("applyElo", () => {
  const elo = new Map([
    ["janniksinner", { name: "Jannik Sinner", elo: { overall: 2319.8, hard: 2263.2, clay: 2215.7, grass: 2088.3 } }],
  ]);

  it("sets elo on matched players, leaves unmatched null", () => {
